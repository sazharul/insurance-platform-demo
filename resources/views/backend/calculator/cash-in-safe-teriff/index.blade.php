...
@section('title', 'Cash in safe and in-transit Tariff Type Manage Page')
                        ...
                        <div class="white_box_tittle list_header">
                            ...
                            <div class="box_right d-flex lms_block">
                                ...
                                <div class="add_button ms-2">
                                    <a href="{{ route('admin.calculator.createCashInSafeTeriff.create') }}"
                                ...
                                <tbody>
                                    @foreach ($teriffs as $teriff)
                                        <tr>
                                            ...
                                            <td>{!! $teriff->propertyLocation->en_name ?? '-' !!}</td>
                                            <td>{!! $teriff->srcc->en_name ?? '-' !!}</td>
                                            <td>{!! $teriff->buildingConstruction->en_class_title ?? '-' !!}</td>
                                            <td>{!! $teriff->value !!}%</td>
                                            <td><a href="{{ route('admin.calculator.statusCashInSafeTeriff.status', $teriff->id) }}"
                                                    class="fw-bolder status_btn {{ $teriff->status == 1 ? '' : 'danger_btn' }}">{!! $teriff->status == 1 ? 'Active' : 'Inactive' !!}</a>
                                            </td>

                                            <td><a href="{{ route('admin.calculator.editCashInSafeTeriff.edit', $teriff->id) }}"
