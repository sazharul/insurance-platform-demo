@php
    $data = Session::get('invoice_details');
@endphp
...
<head>
    ...
    <title>{{ $data['calculator']->{app()->getLocale() . '_name'} }}</title>
        ...
        <div class="table_area" style="padding-left: 5%; padding-right: 5%; margin-top: 30px;">
            <table class="table table-bordered" style="width: 100%; border: 1px solid rgba(0, 0, 0, 0.3);">
                ...
                <tr>
                    ...
                    <td>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Bring the
                            amount of Premium
                            under Motor Certificate No.</p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">EIC/ for
                            one Year Calculated as
                            Under:</p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">SUM INSURED
                            : </p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Tk.
                            {{ $data['vehicle_price'] }}</p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            Manufacturer : </p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            {{ $order_details->motor_year_of_manufacture }}</p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Vehicle
                            Registration # </p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            {{ $order_details->motor_metro }} {{ $order_details->motor_mark }}
                            {{ $order_details->motor_reg_number }}</p>
                        <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Engine #
                        </p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            {{ $order_details->motor_engin_number }} </p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Chassis #
                        </p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            {{ $order_details->motor_chasis_number }}</p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Model Year
                            : </p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            {{ $order_details->motor_year_of_manufacture }}</p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">Seating
                            capability including Driver:</p>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            {{ $data['passenger'] }}+1</p> <br>
                        <h2 style="padding-left: 100px; margin: 0px;">Premium Computation (Tk.)</h2> <br>
                        @if ($data['selected_insurance_type'] == 'Comprehensive')
                            <div class="inside_info_box" style=" padding-left: 100px; display: flow-root;">
                                <div class="inside_left_info" style="float: left; margin-top: 10px; font-size: 18px;">
                                    Basic
                                    Premium</div>
                                <div class="inside_right_info" style="float: right; margin-top: 10px; font-size: 18px;">
                                    Tk.
                                    {{ $data['basic_premium'] }}</div>
                            </div>

                            <div class="inside_info_box" style=" padding-left: 100px; display: flow-root;">
                                <div class="inside_left_info" style="float: left; margin-top: 10px; font-size: 18px;">+
                                    2.65% On F.I.V</div>
                                <div class="inside_right_info"
                                    style="float: right; margin-top: 10px; font-size: 18px;border-bottom: 2px solid rgba(0, 0, 0, 0.3);">
                                    Tk. {{ $data['fiv_amount'] }}</div>
                            </div>

                            @if (isset($data['risk_cover']))
                                <div class="inside_info_box" style=" padding-left: 100px; display: flow-root;">
                                    <div class="inside_left_info"
                                        style="float: left; margin-top: 10px; font-size: 18px;">
                                        @foreach ($data['risk_cover'] as $cover_item)
                                            {{ $loop->iteration > 1 ? ',' : '' }}
                                            <span>{{ $cover_item->{app()->getLocale() . '_subtitle'} }}</span>
                                        @endforeach
                                    </div>
                                    <div class="inside_right_info"
                                        style="float: right; margin-top: 10px; font-size: 18px;">
                                        Tk. {{ $data['covarage_amount'] }}</div>
                                </div>
                            @endif
