@php
    $oo = Session::get('medaclaim');
    $data = $oo;
    // dd($data);
    $visit = explode(',', $order_details->user_visit_country);
    $country = DB::table('calculator_countries')
        ->whereIn('id', $visit)
        ->get();
...
<div class="invoice_area" id="invoice_area" style="width: 100%;">
    <div class="invioce_header" style="padding-left: 10%; padding-right: 10%; text-align:center;">
        ...
        <h2 style="margin: 0px;">PREMIUM BILL {{ strtoupper($data['calculator']->en_name) }}
        </h2>
        ...
        <table class="table table-bordered" style="width: 100%; border: 1px solid rgba(0, 0, 0, 0.3);">
            ...
            <tr>
                ...
                <td>
                    ...
                    @endforeach
                    <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                        Departure Date: {{ $data['user_date_of_departure'] }}</p> <br>
                    <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                        Return Date: {{ $data['user_return_date'] }}</p> <br>
