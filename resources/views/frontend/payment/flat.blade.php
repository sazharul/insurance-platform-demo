@php
    ...
    $visit = explode(',', $order_details->flat_risk_coverage);
    $country = DB::table('calculator_risk_covers')
        ->whereIn('id', $visit)
        ->get();
                ...
                <tr>
                    ...
                    <td>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            District: {{ $data['criteria']->district->en_name }}
                        </p> <br>
                        <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                            Location: {{ $data['criteria']->location->en_name }}
                        </p> <br>
                        @foreach ($country as $c_item)
                            <p style="display: inline-block; margin: 0px; margin-bottom: 10px; font-size: 20px;">
                                {{ $c_item->{app()->getLocale() . '_name'} }}</p> <br>
                        @endforeach



                        {{-- <div class="inside_info_box" style=" padding-left: 100px; display: flow-root;">
                            <div class="inside_left_info" style="float: left; margin-top: 10px; font-size: 18px;">Total Sum Insured</div>
