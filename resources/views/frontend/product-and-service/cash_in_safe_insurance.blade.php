...
@section('title', 'Cash In Safe Insurance')
    ...
    <div class="container-fluid">
        <div class="breadgram-image">
            ...
            <div class="breadgram-hero-area">
                ...
                @else
                @endif
                <h2 class="fw-bold">{{ $safe->{app()->getLocale() . '_name'} }}</h2>
                <h6 class="text-center mt-3"><span class="text-dark"><a class="text-dark fw-bold"
                        ...
                        class="text-success fw-bold">{{ $safe->{app()->getLocale() . '_name'} }}</span></h6>
    ...
    <div class="container cus_mb_20">
        ...
        <div class="fire_container cus_mt_20 cus_mb_20">
            <div class="fire_container_content cash_in_safe_panel">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="panel_type type_institution text-center">
                            @foreach ($safe->calculatorInstitutionType->where('status',1) as $cit_item)
                ...
                <div class="wrap" onclick="spd1()">
                    <select class="form-select filter_card mt-2 mb-2 js-example-basic-single"
                        onchange="getPropertyLocation(this)" aria-label="Default select example">
                        <option value="" selected>Select Location</option>
                        @foreach ($safe->calculatorPropertyLocation->where('status', 1) as $cpl_item)
                            <option value="{{ $cpl_item->id }}" class=" mt-2" data-calculator_id="{{ $safe->id }}"
                                data-property_location_id="{{ $cpl_item->id }}"
                                data-property_location_name="{{ $cpl_item->{app()->getLocale() . '_name'} }}"
                                data-url="{{ route('ps.propertyOrOccupationAndMember') }}">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div class="input-group mb-3">
                            <input class="filter_card filter_card_custom form-control" type="text"
                                placeholder="Enter yearly turnover of your institution" id="yearly_turnover">
                ...
                <div class="wrap" onclick="spd2()">
                    <select class="form-select filter_card mt-2 mb-2 js-example-basic-single"
                        ...
                        @foreach ($safe->calculatorStrikeRiotCivilCommotion->where('status',1) as $csrcc)
                            <option value="{{ $csrcc->id }}" class=" mt-2"
                                onclick="srcc('{{ $csrcc->id }}','{{ $csrcc->{app()->getLocale() . '_name'} }}')"
                                data-calculator_id="{{ $safe->id }}" data-property_location_id="{{ $csrcc->id }}"
                                data-property_location_name="{{ $csrcc->{app()->getLocale() . '_name'} }}"
                                data-url="{{ route('ps.propertyOrOccupationAndMember') }}">
                                {{ $csrcc->{app()->getLocale() . '_name'} }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="row">
                    ...
                    <div class="row mb-3 center_position">
                        @foreach ($safe->calculatorBuildingConstruction->where('status',1) as $cbc_item)
                ...
                <div class="row mt-5">
                    <div class="col-lg-12 col-sm-12 new_btn_setup">
                        ...
                        <a class="btn fire_btn fire_btn_calculate mb-2" href="javascript:;"
                            data-url="{{ route('calculation.cashInSafeAndTransitTeriff') }}" data-bs-toggle="modal"
                            data-bs-target="#mediclaimCalculateModal"
                            onclick="cashInSafeAndTransit(this)">{{ __('Calculate') }}</a>
                    </div>
